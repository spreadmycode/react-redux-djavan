(function (global, factory) {
  if (typeof define === "function" && define.amd) {
    define("/apps/AppTaskboard", ["exports", "Site"], factory);
    ...
    global.appsAppTaskboard = mod.exports;
  }
  ...
  var TaskList = function () {
    function TaskList($el, data) {
      ...
      this.bindCancelBtn();
      this.$taskInput = this.$el.find('.checkbox-custom input');
      this.bindTaskInput();
    ...
    babelHelpers.createClass(TaskList, [{
      ...
      value: function bindAddItemToggle() {
        ...
        this.$addItemToggle.on('click', function () {
          var $this = $(this),
              $input = $('[name="name"]', self.$wrap);
          self.$wrap.toggleClass('action-open');
          $input.val('');
        });

        self.$wrap.on('click.add-item', '.form-control-label', function (e) {
          self.$wrap.removeClass('action-open');
          self.$el.off('click.add-item');
        });
      }
    }, {
      key: "bindAddItemBtn",
      ...
      value: function bindTaskInput() {
        this.$taskInput.on('click', function (e) {
          var $this = $(this),
    ...
    handleDatepicker: function handleDatepicker() {
      ...
      $(document).on('click', '.due-date-delete', function () {
        var $this = $(this),
            $target = $this.closest('.slidePanel').data('slidePanel').target,
            taskData = $target.data('taskInfo');
        if (taskData.duedate.length === 0) return;
        taskData.duedate = '';
        $target.data('taskInfo', taskData);
        var $badgeList = $target.find('.task-badges');
        var badgeList = new BadgeList($badgeList, $target.data('taskInfo'));
    ...
    babelHelpers.createClass(AppTaskboard, [{
      ...
      value: function init() {
        ...
        $.getJSON('../../../assets/data/taskboard.json', function (data) {
          _this3.stageList = new StageList(_this3.$taskboard, data);
        });
      }
    }, {
      key: "bindFloatBtn",
      value: function bindFloatBtn() {
        ...
        this.$floatBtn.on('click', function () {
          $('input', _this4.$model).val('');
          $('option:first', $('select', _this4.$model)).prop("selected", 'selected');
        });
      }
    }, {
      ...
      value: function bindStageCreateBtn() {
        ...
        this.$stageCreateBtn.on('click', function () {
          var $name = $('[name="name"]', self.$model);
          var stageData = {
            title: $name.val(),
            tasks: []
          };
          self.stageList.add(stageData);
        });
      }
    }, {
